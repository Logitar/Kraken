...
internal class OneTimePasswordRepository : Repository, IOneTimePasswordRepository
{
  ...
  {
    return await base.LoadAsync<OneTimePassword>(isDeleted, cancellationToken);
  }
  ...
  {
    return await base.LoadAsync<OneTimePassword>(ids.Select(id => id.StreamId), isDeleted, cancellationToken);
  }
  ...
  public async Task<IReadOnlyCollection<OneTimePassword>> LoadAsync(RealmId? realmId, CancellationToken cancellationToken)
  {
    ...
    return await LoadAsync<OneTimePassword>(streamIds.Select(value => new StreamId(value)), cancellationToken);
  }

...
internal class UpdateApiKeyCommandHandler : IRequestHandler<UpdateApiKeyCommand, ApiKeyModel?>
{
  ...
  public async Task<ApiKeyModel?> Handle(UpdateApiKeyCommand command, CancellationToken cancellationToken)
  {
    ...
    ApiKeyId apiKeyId = new(_applicationContext.RealmId, command.Id);
    ...
    IReadOnlyDictionary<string, Role> roles = await _roleManager.FindAsync(apiKey.RealmId, payload.Roles.Select(action => action.Role), cancellationToken);

...
public class UnpublishContentCommandHandlerTests
{
  ...
  {
    UnpublishContentCommand command = new(_content.EntityId, _language.EntityId);
  ...
  {
    UnpublishContentCommand command = new(_content.EntityId);
    ContentModel? model = await _handler.Handle(command, _cancellationToken);
    Assert.NotNull(model);
    Assert.Same(_model, model);
  ...
  {
    UnpublishContentCommand command = new(_content.EntityId, languageId: null);
    ContentModel? model = await _handler.Handle(command, _cancellationToken);
    Assert.NotNull(model);
    Assert.Same(_model, model);
  ...
  public async Task Given_LocaleNotFound_When_Handle_Then_NullReturned()
  {
    ...
    UnpublishContentCommand command = new(content.EntityId, Guid.NewGuid());

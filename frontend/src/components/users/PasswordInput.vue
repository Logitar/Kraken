...
const rules = computed<ValidationRules>(() => {
  ...
  if (props.confirm) {
    ...
    if (props.settings.requiredLength) {
      rules.min_length = props.settings.requiredLength;
    }
    if (props.settings.requiredUniqueChars) {
      rules.unique_chars = props.settings.requiredUniqueChars;
    }
